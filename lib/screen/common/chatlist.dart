...
class _chatlistsState extends State<chatlists> {
  ...
  Widget build(BuildContext context) {
    ...
    if(private){
      ...
      return FutureBuilder<QuerySnapshot>(future: store.collection('classrooms').where('teacher',isEqualTo:_auth.currentUser!.uid).get(),builder: (context,classdetails){
                      ...
                      child:Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            (classorder.length>0)?Container(
                                height: MediaQuery.of(context).size.height*0.7,
                                width: MediaQuery.of(context).size.width*0.9,
                                padding: EdgeInsets.all(20),

                                child: FutureBuilder<QuerySnapshot>(future:store.collection('classrooms').where(FieldPath.documentId,whereIn: classorder).get(),builder: (context,snap){
                                  if(snap.connectionState==ConnectionState.waiting){
                                    return Container();
                                  }else{
                                    List details =snap.data!.docs;
